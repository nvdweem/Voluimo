...
//static 
AudioSessionService& AudioSessionService::instance()
{
	if (!__instance)
	{
...
HRESULT AudioSessionService::RefreshAudioSessions()
{
	...
	{
		HRESULT __hr = device->Activate(__uuidof(IAudioSessionManager2), CLSCTX_INPROC, nullptr, (void**)&audioSessionManager);
		if (FAILED(__hr)) 
		{ 
			return __hr; 
		}
...
HRESULT AudioSessionService::CreateEtAudioSessionFromAudioSession(CComPtr<IAudioSessionEnumerator> audioSessionEnumerator, int sessionCount, EarTrumpetAudioSession* etAudioSession)
{
    ...
    etAudioSession->SessionId = (unsigned long)stringHash(static_cast<PWSTR>(sessionIdString));
...
HRESULT AudioSessionService::GetAudioSessions(std::vector<EarTrumpetAudioSession>& sessions)
{
	sessions = _sessions;
	return S_OK;
}
